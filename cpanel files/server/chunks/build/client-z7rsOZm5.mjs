import { f as capitalizeFirstLetter, h as getBaseURL, i as createFetch } from "./auth-DnREO_GR.mjs";
import { aP as createEnv, ay as string } from "./db-COtzJr4P.mjs";
...
const useAuthQuery = (initializedAtom, path, $fetch2, options) => {
...
function createSessionRefreshManager(opts) {
  const { sessionAtom, sessionSignal, $fetch: $fetch2, options = {} } = opts;
...
function getSessionAtom($fetch2, options) {
  const $signal = /* @__PURE__ */ atom(false);
  const session = useAuthQuery($signal, "/get-session", $fetch2, { method: "GET" });
  onMount(session, () => {
...
const getClientConfig = (options, loadEnv) => {
  ...
  const { $sessionSignal, session } = getSessionAtom($fetch2, options);
  ...
  for (const plugin of plugins) {
    if (plugin.getAtoms) Object.assign(pluginsAtoms, plugin.getAtoms?.($fetch2));
  ...
  for (const plugin of plugins) if (plugin.getActions) Object.assign(pluginsActions, plugin.getActions?.($fetch2, $store, options));
  return {
    get baseURL() {
      return baseURL;
    },
...
function createAuthClient(options) {
  const { pluginPathMethods, pluginsActions, pluginsAtoms, $fetch: $fetch2, atomListeners, $store } = getClientConfig(options);
  let resolvedHooks = {};
  ...
  return createDynamicPathProxy({
    ...
    $fetch: $fetch2,
    $store
  }, $fetch2, pluginPathMethods, pluginsAtoms, atomListeners);
}
const __vite_import_meta_env__ = { "BASE_URL": "/", "DEV": false, "MODE": "production", "PROD": true, "SSR": true, "TSS_CLIENT_OUTPUT_DIR": "dist/client", "TSS_DEV_SERVER": "false", "TSS_ROUTER_BASEPATH": "", "TSS_SERVER_FN_BASE": "/_serverFn/", "VITE_BASE_URL": "http://localhost:3000" };
const env = createEnv({
  clientPrefix: "VITE_",
  client: {
    VITE_BASE_URL: string().url().default("http://localhost:3000"),
    VITE_STRIPE_PUBLISHABLE_KEY: string().startsWith("pk_").optional()
  },
