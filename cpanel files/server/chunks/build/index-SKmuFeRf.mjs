import { c as createAdapterFactory, s as sql } from "./auth-DnREO_GR.mjs";
import { b, g } from "./auth-DnREO_GR.mjs";
      ...
      return {
        ...
        async findOne({ model, where, select, join }) {
          ...
          let query = db$1.selectFrom((eb) => {
            ...
            if (and) b2 = b2.where((eb$1) => eb$1.and(and.map((expr) => expr(eb$1))));
            if (or) b2 = b2.where((eb$1) => eb$1.or(or.map((expr) => expr(eb$1))));
            return b2.selectAll().as("primary");
          }).selectAll("primary");
        ...
        async findMany({ model, where, limit, offset, sortBy, join }) {
          ...
          let query = db$1.selectFrom((eb) => {
            ...
            if (config?.type === "mssql") {
              if (offset !== void 0) {
                if (!sortBy) b2 = b2.orderBy(getFieldName({
                  model,
                  field: "id"
                }));
                b2 = b2.offset(offset).fetch(limit || 100);
              } else if (limit !== void 0) b2 = b2.top(limit);
            } else {
              if (limit !== void 0) b2 = b2.limit(limit);
              if (offset !== void 0) b2 = b2.offset(offset);
            }
            if (sortBy?.field) b2 = b2.orderBy(`${getFieldName({
              model,
              field: sortBy.field
            })}`, sortBy.direction);
            if (and) b2 = b2.where((eb$1) => eb$1.and(and.map((expr) => expr(eb$1))));
